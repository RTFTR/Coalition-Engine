...
for (var i = 0, n = instance_number(obj_ParentEnemy), enemy_find; i < n; ++i) {
	enemy_find[i] = instance_find(obj_ParentEnemy, i);
	if enemy_find[i].is_dying
		state = 0.6;
}
var _turn = obj_BattleController.battle_turn - 1;
...
if state == 1 or (state == 2 and dialog_at_mid_turn) and !died and !is_spared
{
	...
	{
		obj_BattleController.begin_turn();
		exit;
	}
	...
	if dialog_text_typist.get_state() == 1 {
		if input_check_pressed("confirm") {
			...
			var text = (obj_BattleController.battle_turn < array_length(dialog_text) and state == 1) ?
				dialog_text[obj_BattleController.battle_turn] : "";
	...
	if is_being_attacked {
		...
		else
		{
			...
			if death_time = 1 + attack_end_time + dust_speed + 60 {
				...
				global.data.Kills++;
				RemoveEnemy();
				if array_length(obj_BattleController.enemy_instance) == 0 obj_BattleController.end_battle();
			}
		}
	}
...
if is_being_spared {
	if !died and!is_spared
		if enemy_is_spareable {
			if spare_function == -1
			{
				wiggle = false;
				//Add Reward
				obj_BattleController.Result.Gold += Gold_Give;
				obj_BattleController.Result.Exp += Exp_Give;
				is_spared = true;
				audio_play(snd_vaporize);
	...
	for (var i = 0, n = instance_number(obj_ParentEnemy), continue_battle = false, enemy_find; i < n; ++i) {
		enemy_find[i] = instance_find(obj_ParentEnemy, i);
		if !enemy_find[i].is_spared continue_battle = true;
	}
	if !continue_battle
		obj_BattleController.end_battle();
	else
	{
		if spare_end_begin_turn {
			//Begins turn if it's set to be
			if !is_spared {
				obj_BattleController.dialog_start();
			}
		}
	}
...
if is_spared and image_alpha == 0.5 {
	//Remove enemy
	enemy_in_battle = false;
	if array_length(obj_BattleController.enemy_instance) == 0 RemoveEnemy();
}

if state == 2 {
	draw_set_halign(fa_right);
	draw_set_color(c_white);
	if obj_BattleController.debug
