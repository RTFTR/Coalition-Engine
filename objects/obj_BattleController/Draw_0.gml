...
if battle_state == BATTLE_STATE.MENU {
	...
	if menu_state == BATTLE_MENU_STATE.BUTTON_SELECTION or menu_state == -1 {
	...
	if is_val(menu_state, BATTLE_MENU_STATE.FIGHT, BATTLE_MENU_STATE.ACT) // Fight - Act
	{
		...
		repeat(array_length(enemy_name)) // Draw enemy hp bar in Fight state
		{
			...
			if instance_exists(enemy[i]) // Check if the enemy slot is valid before name drawing
			{
				...
				var xwrite = 450;
				if menu_state == BATTLE_MENU_STATE.FIGHT and enemy_draw_hp_bar[i] == 1 {
					decrease_y -= 32;
	...
	if menu_state == BATTLE_MENU_STATE.ITEM // Item list
	{
		Item_Info_Load();
	...
	if menu_state == BATTLE_MENU_STATE.ACT_SELECT // Draw Act Texts
	{
	...
	if menu_state == BATTLE_MENU_STATE.FIGHT_AIM //Fight Anim
	{
